...
如果你不喜欢默认的轻量合成按键音，可以在扩展的完整设置里导入本地音乐。导入后，小人会播放这首本地背景音乐，并根据打字速度自动调节音量：慢速轻一点，正常更明显，快速更有力度。
...
- 在扩展弹窗点「打开完整设置…」→「本地背景音乐」导入一首音频；导入后默认合成按键音会停止，只播放你的本地背景音乐，音量随 slow / normal / fast 变化。
